...
const addTreinoHandler = async (req, res) => {
  ...
  if (!aluno_id || !descricao) return res.status(400).json({ error: 'Dados incompletos' });
  if (descricao.trim().length < 20)
    return res.status(400).json({ error: 'Descrição deve ter pelo menos 20 caracteres' });

  // se vier '', transformamos em undefined para o DB aplicar now()
  const dt = typeof data_inicio === 'string' && data_inicio.trim() === '' ? undefined : data_inicio;

  try {
    const novoTreino = await addTreino(descricao, dt, aluno_id);
...
const updateTreinoHandler = async (req, res) => {
  ...
  if (!descricao) return res.status(400).json({ error: 'Dados incompletos' });
  if (descricao.trim().length < 20)
    return res.status(400).json({ error: 'Descrição deve ter pelo menos 20 caracteres' });

  // regra: se vier '', não altera a data (passa undefined)
  const dt = typeof data_inicio === 'string' && data_inicio.trim() === '' ? undefined : data_inicio;

  try {
    const treinoAtualizado = await updateTreino(id_treino, descricao, dt);
